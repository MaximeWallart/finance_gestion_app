import 'package:finance_gestion_app/modified/remove_over_scroll.dart';
...
class _AwesomeDropDownState extends State<AwesomeDropDown>
  ...
  Widget build(BuildContext context) {
    if (widget.isBackPressedOrTouchedOutSide && _isDropdownOpened) {
      _openAndCloseDrawer();
    ...
    return WillPopScope(
      ...
      child: GestureDetector(
        ...
        onTap: () {
          if (_isFirstTime || (!widget.isPanDown)) {
            setState(() {
              if (widget.isPanDown) {
                widget.isPanDown = false;
              }
  ...
  OverlayEntry _createFloatingDropdown(int numOfListItemToShow) {
    int numOfListItem = numOfListItemToShow <= 10 &&
